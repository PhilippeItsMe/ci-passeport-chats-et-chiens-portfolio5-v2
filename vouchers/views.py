...
def generate_single_voucher(request, business_id, discount_type):
    ...
    voucher, created = Voucher.objects.get_or_create(
        ...
        defaults={'discount_value': 50.00
                  if discount_type == 'percentage' else 20.00,
                  'date_expires': now() + timedelta(days=90)}
    )

    # Redirect if PDF already exists
    if not created and voucher.pdf_file:
        return redirect(voucher.pdf_file.url + '.pdf'
                        if hasattr(
                            voucher.pdf_file, 'url') else voucher.pdf_file)
    ...
    if not html_string.strip():
        return HttpResponse(
            "PDF generation failed: Empty HTML template.", status=500)
    ...
    try:
        ...
        voucher.pdf_file = cloudinary_response.get(
            'secure_url', cloudinary_response.get('url'))
        voucher.save(update_fields=['pdf_file'])

    except Exception as e:
        return HttpResponse(
            f"Error saving PDF to Cloudinary: {str(e)}", status=500)
